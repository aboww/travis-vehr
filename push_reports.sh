...
sed -i 's^<!--template-->^<!--template-->\n<a href="https://github.com/aboww/travis-vehr/commit/'"$COMMIT_ID"'">'"$COMMIT_SHORT"'(VEHR)</a><span> :  ('"$BUILD_DATE_TIME"')</span> <a href="site_'"$COMMIT_SHORT"'/surefire-report.html">Surefire test reports</a> <a href="cucumber_'"$COMMIT_SHORT"'/overview-features.html">Cucumber test reports</a> <a href="https://travis-ci.org/aboww/travis-vehr/builds/'"$TRAVIS_BUILD_ID"'">Travis CI build</a> <a href="https://api.travis-ci.org/v3/job/'"$TRAVIS_JOB_ID"'/log.txt">Travis CI build log(raw)</a><br>^g' travis-reports/index.html
